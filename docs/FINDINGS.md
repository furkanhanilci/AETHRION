...
## 9. Findings from the 2026-08-24 collaboration-backend adoption

Applying an external architecture-change specification surfaced three defects in
this repository. None was in the specification; all three were here already.

| # | Finding | Where | State |
|---|---|---|---|
| **O1** | **Five figure generators were never run by the command that regenerates figures.** `make_figures.py` listed sixteen modules while `docs/figures/` held twenty-one SVGs, so `fig_compiler`, `fig_context`, `fig_decision`, `fig_disciplines` and `fig_reproduction` were regenerated by nobody. The drift check compares each SVG to itself, so it reported **21 figures, 0 drift** throughout — five figures could have been hand-edited, or left stale by a change to their generator, and the bundle would have agreed with itself. Found only because an edit to `fig_context.py` produced no change in the figure it draws | `scripts/make_figures.py` | **Closed.** The module list is checked against `scripts/fig_*.py` on every run and a generator absent from it fails the build. `tests/test_collaboration_boundary.py` pins the equality |
...
> named the decisions it was deliberately not making, and said explicitly that
> its own baseline commit might have moved. The one place it overreached — a
> direct adaptation under a licence it reported but nobody here could read — was
> caught by `check_upstream_lineage.py` R7 and downgraded to the mode that moves
> no files.
