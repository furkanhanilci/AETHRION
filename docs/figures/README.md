...
| Scope | The twenty-one generated figures, their design system and their guarantees |
...
**In one paragraph.** Figures here are generated artifacts, not drawings: the corpus is the source, the generator is version-controlled, and the SVG is reproducible from a clean checkout. There are twenty-one of them rather than one per document, because a figure earns its place only by carrying a mechanism prose carries badly. Every string is measured against the box it sits in, and a figure that cannot be laid out honestly fails the build.
...
> **A figure the drift check could not see.** For several baselines
> `make_figures.py` listed sixteen generators while this directory held
> twenty-one figures, so five of them — compiler, context, decision,
> disciplines, reproduction — were never regenerated by the command that claims
> to regenerate them. The bundle reported *21 figures, 0 drift* the whole time,
> because the drift check compares each SVG to itself. It was found when an edit
> to `fig_context.py` produced no change in the figure it draws. The list is now
> checked against the generators on disk, and a generator absent from it fails
> the build rather than being silently skipped.

## 1. Why there are twenty-one figures and not fifty
...
| 20 | `aethrion_backend.svg` | Two AETHRION-owned contracts with everything adopted *underneath* them, beside the test that decides whether the boundary is real — remove the substrate and say what disappears | An integration diagram, which shows what connects to what and never what may decide what |
| 21 | `aethrion_runtime.svg` | That a cognitive function is chosen before a runtime is, over the five levels — role, function, model, runtime, identity — with the failure that follows from collapsing each into the next | "role is not runtime", which is unarguable, unmemorable and gets violated the first time a harness is convenient |
...
Applied identically to all twenty-one figures.
...
None of the twenty-one is a generic left-to-right pipeline, because none of the
