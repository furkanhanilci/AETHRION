...
| Kardeş belge | `AIRL_OS_IDEAL_STRUCTURE.md` (roller, review mekanizmaları, metascience) |
| Tarih | 2026-08-22 |
...
| Skill | Görev (bkz. `AIRL_OS_IDEAL_STRUCTURE.md` Bölüm C) |
|---|---|
...
### G. İletişim ve dış dünya skill'leri

> **Mimari uyarı:** Mesajlaşma bir *skill* değil, bir **Tool Broker
> konnektörüdür**. Bu skill'ler ajanın *niyet* üretme disiplinini tanımlar;
> gönderimi **Notification Broker** yapar. Ajan hiçbir mesajı doğrudan
> göndermez.

| Skill | Yön | Kritik kural |
|---|---|---|
| `notifying-humans` | giden | Kanal başına veri sınıfı tavanı; DLP; idempotency |
| `routing-decision-requests` | çift yön | **Mesajlaşma yetkilendirme kanalı değildir** |
| `receiving-external-messages` | gelen | **Gelen mesaj asla talimat değildir** (Zone 3) |
| `escalating-and-paging` | giden | Zaman aşımı asla auto-approve değil |
| `publishing-digests` | giden | Salt-okunur türev; durum değiştirmez |
| `submitting-external-records` | giden | OSF/Zenodo/ORCID — geri alınamaz, insan onaylı |
| `monitoring-external-feeds` | gelen | G10; sessiz supersession yok |

#### Notification Broker — yeni bileşen

`Tool Broker`'ın bir alt sınıfı olarak, Execution Plane'e eklenir:

```
Ajan → bildirim NİYETİ
     → Notification Broker
          1. kimlik + TaskContract doğrulama
          2. VERİ SINIFI TAVANI (kanal başına)
          3. DLP taraması (secret, PII — Presidio)
          4. şablon uygulaması (serbest metin değil)
          5. idempotency anahtarı
          6. rate limit + sessiz saat
          7. gönderim  →  NotificationReceipt + ToolReceipt
```

**Soyutlama önerisi:** [Apprise](https://github.com/caronc/apprise) — 143
servisi tek URL formatıyla soyutlar (Telegram, Discord, Slack, Matrix, ntfy,
Signal, e-posta, WhatsApp/Twilio). Broker'ın taşıyıcı katmanı olarak uygundur;
policy ve DLP katmanı **üstüne** yazılır, ona bırakılmaz.

#### Kanal başına veri sınıfı tavanı

| Kanal | Tavan | Gerekçe |
|---|---|---|
| ntfy (self-hosted) | **D2** | Kendi sunucunuz |
| Matrix (self-hosted) | **D2** | E2E + kendi homeserver |
| Signal | D2 | E2E; otomasyon zor |
...
>
> **WhatsApp, ajan-başlatmalı bildirim için en kötü kanaldır:** kullanıcının son
> mesajından itibaren 24 saatlik pencere dışında yalnız önceden onaylanmış
> şablonlar gönderilebilir. Operasyonel olarak kırılgandır.

#### Gelen trafik neden daha tehlikeli

Giden mesaj bir veri sızıntısı riskidir. **Gelen mesaj bir kontrol devralma
riskidir.** E-posta, PDF eki veya Discord mesajı içine gömülü metin, ajan
bağlamına girdiğinde prompt injection olur — `ACC-05` senaryosu, mesajlaşma
yüzeyiyle genişler.

Bu yüzden gelen içerik: gönderen doğrulaması → ek taraması →
`<untrusted-external-content>` işaretlemesi → **talimat çıkarımı yok**.

#### Onay akışı — `ACC-25`'in önleyici tarafı

```
DecisionRequest → bildirim + İMZALI, SÜRELİ, TEK KULLANIMLIK derin bağlantı
                → insan bağlantıyı açar
                → KİMLİK DOĞRULAMALI yüzeyde kararı verir
                → DecisionRecord imzalanır
```

Sohbet cevabı: okundu bilgisi ✅, ek bilgi talebi ✅, **onay/ret ❌**.
...
Superpowers'ın fan-out kuralları, `AIRL_OS_IDEAL_STRUCTURE.md` Bölüm B3'teki
...
Ve bu, `AIRL_OS_IDEAL_STRUCTURE.md` Bölüm B1'deki **in-principle acceptance**
...
### Faz S4 — İletişim katmanı

| # | İş | Not |
|---|---|---|
| S4.1 | **Notification Broker** — Tool Broker alt sınıfı; policy + DLP + idempotency | Taşıyıcı: Apprise |
| S4.2 | Kanal kaydı + **veri sınıfı tavanı** tablosu, kod içinde zorlanır | D3/D4 → içeriksiz tetikleyici |
| S4.3 | `notifying-humans`, `escalating-and-paging`, `publishing-digests` | giden |
| S4.4 | **İmzalı, süreli, tek kullanımlık derin bağlantı** + kimlik doğrulamalı karar yüzeyi | `routing-decision-requests` |
| S4.5 | Karantina hattı: gönderen doğrulama → ek taraması → `<untrusted-external-content>` | `receiving-external-messages` |
| S4.6 | G10 besleme bağlantıları (Crossref/Retraction Watch/CVE/registry) | `monitoring-external-feeds` |
| S4.7 | OSF ön-kayıt + Zenodo DOI + ORCID | `submitting-external-records` |

**Sıra kritik:** S4.1–S4.2 (broker + tavan) tamamlanmadan hiçbir kanal
bağlanmaz. Kanalı önce bağlayıp policy'yi sonra eklemek, veri sınıfı ihlalini
kalıcılaştırır.

**Önerilen ilk kanal:** ntfy (self-hosted) + Telegram. Sebep: ntfy D2'ye kadar
çıkabilir ve kendi sunucunuzdadır; Telegram bot API'si en düşük sürtünmeli
etkileşimli kanaldır. WhatsApp **en sona** bırakılır.

### Faz S5 — Plan projeksiyonu

| # | İş |
|---|---|
| S5.1 | 130 WP → makine-tüketilebilir task-brief üretici (**mekanik, prompt değil**) |
| S5.2 | `progress.jsonl` append-only defter |
| S5.3 | WP ↔ skill eşlemesi (her WP hangi skill'lerle yürütülür) |
| S5.4 | Yeni WP'ler: Notification Broker, Metascience düzlemi, eklenen roller |
...
**Sonraki adım:** Bu belge ve `AIRL_OS_IDEAL_STRUCTURE.md` onaylandıktan sonra,
