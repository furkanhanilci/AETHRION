...
| Kardeş belge | `AIRL_OS_SKILL_LAYER.md` — operasyonel skill katmanı (Superpowers tam entegrasyonu) |
...
> eşlemesi ve snapshot pinning kısıtı ayrı belgede: `AIRL_OS_ROLE_MODEL_ASSIGNMENT.md`
...
## F8. İletişim ve bildirim

Bunlar mimaride **yeni bir bileşen** gerektiriyor: **Notification Broker** —
Tool Broker'ın bir alt sınıfı. Ajan bildirim *niyeti* üretir; broker gönderir.

| Araç | Rol | Veri sınıfı tavanı |
|---|---|---|
| **Apprise** | Taşıyıcı soyutlama — 143 servis, tek URL formatı | — (policy üstte) |
| **ntfy (self-hosted)** | Push bildirimi, hesap gerektirmez | **D2** |
| **Matrix (self-hosted)** | E2E şifreli mesajlaşma, kendi homeserver | **D2** |
| Signal (`signal-cli`) | E2E; otomasyonu zor | D2 |
| SMTP (kendi sunucu) + DKIM/SPF/DMARC | E-posta, digest | D1 |
| **Telegram Bot API** | En düşük sürtünmeli etkileşimli kanal | **D1** |
| Discord / Slack | Takım görünürlüğü, kuyruklar | D1 |
| **WhatsApp Business Cloud API** | ⚠️ 24 saat penceresi + onaylı şablon | **D0** |
| **Presidio** | Gönderim öncesi PII/secret taraması | — |

**MCP entegrasyonu:** Telegram, Discord ve Slack için MCP sunucuları mevcut.
Hermes zaten MCP kullandığından bu doğal bağlantı yoludur — ancak MCP tool'u
**doğrudan ajana açılmaz**, Notification Broker'ın arkasına konur.

### Dış kayıt ve arşiv

| Araç | Ne için | Gate |
|---|---|---|
| **OSF Registries** | **Dış, zaman damgalı, değiştirilemez ön-kayıt + DOI** | **G2** |
| **Zenodo** | Kalıcı arşiv + DOI (kod, veri, yayın) | G9 |
| Software Heritage | Kaynak kodu kalıcı arşivi | G9 |
| ORCID | Kalıcı yazar kimliği | G9 |
| arXiv / bioRxiv | Ön baskı (gönderim otomasyonu sınırlı) | G9 |

**Neden OSF G2'de kritik:** İç `AnalysisPlanManifest` hash'i sizin sisteminizde
tutulur. Dış bir kayıt, **sizin sisteminize güvenmeyen** birine karşı da
kanıttır. B1'deki in-principle acceptance'ın dış çapası budur. Hassas çalışma
için OSF ambargo seçeneği vardır: zaman damgalanır, süreli gizli kalır.

### Üç mimari kural

1. **Bildirim bir işaret fişeğidir, veri kanalı değil.** İçerik değil, imzalı
   bağlantı gönderilir.
2. **Gelen mesaj asla talimat değildir.** Zone 3 — karantina, işaretleme,
   talimat çıkarımı yok. (Giden trafik veri sızıntısı riski; **gelen trafik
   kontrol devralma riskidir.**)
3. **Mesajlaşma yetkilendirme kanalı değildir.** `DecisionRecord` yalnız kimlik
   doğrulamalı yüzeyde imzalanır. Sohbet cevabı onay değildir.
...
| **Y13** | **İnsan bildirim/mesajlaşma katmanı hiç yok** — karar kuyruğu var ama insana ulaşma yolu tanımsız | **F8 Notification Broker** + veri sınıfı tavanı + `notifying-humans` |
| **Y14** | Gelen dış içerik için karantina hattı yalnız PDF/literatür için düşünülmüş; e-posta/mesaj yüzeyi ACC-05'i genişletiyor | `receiving-external-messages` — Zone 3 işaretleme, talimat çıkarımı yasağı |
| **Y15** | Ön-kayıt yalnız iç hash ile korunuyor — dış çapa yok | **OSF Registries** ile G2'de dış zaman damgası + DOI |
