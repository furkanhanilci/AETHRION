...
([`ADR-010`](docs/architecture/ADR-010_policy_backend.md)) — and AgentDojo named
...
Every entry in [`provenance/README.md`](provenance/README.md) states its upstream,
...
Rules: [`ADR-004`](docs/architecture/ADR-004_mechanism_assimilation.md). Which
mechanism came from where:
[`AETHRION_RELATED_SYSTEMS.md`](docs/architecture/AETHRION_RELATED_SYSTEMS.md) §5.1.
...
flowchart LR
    subgraph WORKING["RUNNING - verified locally"]
        W["Zotero read-only client<br/>SQLite source registry<br/>Obsidian projection<br/>Hermes MCP, 5 tools<br/>systemd units · 149 tests<br/>plan seal · 20 status checks<br/>signed evidence manifest<br/>17 generated figures<br/>upstream lineage register + checker"]
    end
...
*Figure 8 — One command runs all twenty. Each row states the claim the check earns
...
```
149/149 tests pass · plan seal 632/632 OK · plan semantics OK · service and timer active
...
Figures: 17/17 match their generators; 0 text overflows out of their boxes
