...
# AETHRION — Ideal Structure Proposal, Contributions and Architecture Review
...
| Sibling document | `AETHRION_SKILL_LAYER.md` — the operational skill layer (full Superpowers integration) |
...
**In AETHRION — and this can be enforced mechanically:**
...
**In AETHRION:** before the G4 (Baseline & Budget) approval, the Red Team runs a
...
**In AETHRION:** a `severity_assessment` for every `ClaimVersion`:
...
**In AETHRION:** if a `DisagreementCase` does not close through arbitration, then
...
> `AETHRION_ROLE_MODEL_ASSIGNMENT.md`.
...
| Superpowers | AETHRION | The shared insight |
|---|---|---|
