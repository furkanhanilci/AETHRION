---
title: "WP-118 — Operational Readiness, On-Call and Runbook Simulation"
aliases:
  - "WP-118"
...
status: NOT_STARTED
summary: "The service owner, on-call, escalation, incident command, break-glass, backup/restore, reconciliation, security and business continuity runbooks have all been executed in staging."
source: "planning/commissioning/10_INTEGRATION_CUTOVER/WP-118_operational_readiness.md"
generated: true
...
| [Test procedures](wp_118_operational_readiness.tests.md) | How is it tested, in what environment, with what data, and what counts as a complete run? | The implementer and the tester |
| [Acceptance criteria](wp_118_operational_readiness.acceptance.md) | What must hold for this to be `ACCEPTED`, and what does it still not establish? | The independent verifier |
...
**117 of 141 packages (83%)** must reach `ACCEPTED` before this one can begin — the direct list above plus everything they in turn require. This is the number that determines when the package can actually start; the direct list is only its last layer.
...
- **Transitively reachable:** **12 of 141 packages (9%)** cannot be accepted until this one is.
...
The outline below is the summary. The executable procedure — environment, data, coverage items, cases, execution log, incident and completion reports — is in [`WP-118_operational_readiness.tests.md`](wp_118_operational_readiness.tests.md).
...
The programme-level conditions are below. The package-specific, measurable criteria — each with a threshold and the test case that decides it — are in [`WP-118_operational_readiness.acceptance.md`](wp_118_operational_readiness.acceptance.md), together with what this package still cannot establish.
