---
title: "WP-117 — Performance, Capacity and Load Commissioning"
aliases:
  - "WP-117"
...
status: NOT_STARTED
summary: "Under the approved workload envelope, the intake/gate, event, model, broker, registry, experiment, review and impact queues meet their SLO, quota and cost limits."
source: "planning/commissioning/10_INTEGRATION_CUTOVER/WP-117_performance_capacity.md"
generated: true
...
| [Test procedures](wp_117_performance_capacity.tests.md) | How is it tested, in what environment, with what data, and what counts as a complete run? | The implementer and the tester |
| [Acceptance criteria](wp_117_performance_capacity.acceptance.md) | What must hold for this to be `ACCEPTED`, and what does it still not establish? | The independent verifier |
...
Whether the system holds at the workload it claims to support — and the workload
envelope is the deliverable, not the load test.

### The envelope must be approved before it is measured (T01)

A capacity test run without a declared envelope measures whatever the tester chose.
`00_PROGRAM/10`'s entry condition is *capacity/load test results meet the **approved**
workload envelope*, so the envelope is a governance artifact.

### The assurance queue and the human SLA are the real constraints (T05)

Most capacity work stops at services. `00_PROGRAM/08` states the binding one:
**human decision capacity is a hard quota, not a throughput target**, and
`00_PROGRAM/07`'s `PR-04` names verification backlog as critical.

A system that scales its research throughput without scaling its review capacity
produces exactly the pressure that ends in a bypass request.

### Backpressure is the correct behaviour, not an error (T04)

At the envelope's edge, the system should queue and signal — not fail, and not
silently degrade. `00_PROGRAM/01` invariant 9 is the same shape for budget: pause
without losing state.

### The cost curve makes capacity a portfolio decision (T06)

Throughput and cost together. A capacity plan that ignores the cost curve produces
a system that can run more work than the budget can pay for, and the constraint is
discovered by `PR-09` rather than by planning.

### Fan-out is the multiplier that breaks envelopes

A workload mix that looks modest becomes large when council review and parameter
sweeps multiply it. The envelope must state the fan-out assumption explicitly.
...
**116 of 160 packages (72%)** must reach `ACCEPTED` before this one can begin — the direct list above plus everything they in turn require. This is the number that determines when the package can actually start; the direct list is only its last layer.
...
- **Directly unblocked:** 4 — `WP-118` · `WP-119` · `WP-120` · `WP-127`
...
| Controls | `CTL-CST-01` · `CTL-OBS-01` |
...
- Dependencies accepted: [WP-053 — Kueue Queue, Quota and Priority Policy](../06_EXECUTION_SECURITY/wp_053_kueue_quota.md), [WP-096 — OpenTelemetry End-to-End Correlation Spine](../09_EXPERIENCE_OBSERVABILITY/wp_096_otel_correlation.md), [WP-098 — Grafana and the Six Operational Graphs](../09_EXPERIENCE_OBSERVABILITY/wp_098_grafana_six_graphs.md), [WP-100 — Cost Ledger, Budget Envelopes and FinOps](../09_EXPERIENCE_OBSERVABILITY/wp_100_cost_ledger_finops.md), [WP-101 — Service Catalogue, SLOs and Alert/Runbook Binding](../09_EXPERIENCE_OBSERVABILITY/wp_101_service_slo_alerting.md), [WP-115 — Full System Regression and Commissioning Dossier](../10_INTEGRATION_CUTOVER/wp_115_full_system_regression.md)
...
- **Capacity Engineering Lead** carries the acceptance decision; **SRE / FinOps / Assurance** must verify independently of whoever implements.
...
Neither register binds an upstream mechanism or a runtime component to `WP-117`, so every deliverable below is **`BUILD_NATIVE`**.
...
The outline below is the summary. The executable procedure — environment, data, coverage items, cases, execution log, incident and completion reports — is in [`WP-117_performance_capacity.tests.md`](wp_117_performance_capacity.tests.md).
...
The programme-level conditions are below. The package-specific, measurable criteria — each with a threshold and the test case that decides it — are in [`WP-117_performance_capacity.acceptance.md`](wp_117_performance_capacity.acceptance.md), together with what this package still cannot establish.
