---
title: "WP-026 — Content-Addressed Object Store and WORM"
aliases:
  - "WP-026"
...
status: NOT_STARTED
summary: "PDF, dataset, artifact, evidence and publication bytes are stored immutably under a content hash, with object lock, encryption, retention and legal-hold."
source: "planning/commissioning/03_FOUNDATION/WP-026_object_store_worm.md"
generated: true
...
| [Test procedures](wp_026_object_store_worm.tests.md) | How is it tested, in what environment, with what data, and what counts as a complete run? | The implementer and the tester |
| [Acceptance criteria](wp_026_object_store_worm.acceptance.md) | What must hold for this to be `ACCEPTED`, and what does it still not establish? | The independent verifier |
...
| `CMP-015` — S3 Object Lock semantics | `OPTIONAL_BACKEND` | Compliance-mode WORM retention enforcement. | The `ImmutableObjectStore` contract, and the proof that delete and overwrite are actually refused. | **1** |
| `CMP-016` — lakeFS | `OPTIONAL_BACKEND` | Git-like branching over object storage. | The separation itself — working datasets are not accepted evidence. | **1** |
...
| `CMP-015` | The store refuses mutation; it does not decide what deserves to be immutable. Retention class is a policy decision recorded before the object is written. | Governance mode, and any bucket configuration in which an administrator can shorten retention. |
| `CMP-016` | A working dataset branch is never an evidence reference. Nothing on a mutable branch may be cited by a claim; promotion into the immutable store is an explicit, recorded act. | lakeFS as the store for accepted evidence. |
...
**`CMP-015` — S3 Object Lock semantics** · `OPTIONAL_BACKEND` · status `PROPOSED`
...
**`CMP-016` — lakeFS** · `OPTIONAL_BACKEND` · status `PROPOSED`
...
The outline below is the summary. The executable procedure — environment, data, coverage items, cases, execution log, incident and completion reports — is in [`WP-026_object_store_worm.tests.md`](wp_026_object_store_worm.tests.md).
...
The programme-level conditions are below. The package-specific, measurable criteria — each with a threshold and the test case that decides it — are in [`WP-026_object_store_worm.acceptance.md`](wp_026_object_store_worm.acceptance.md), together with what this package still cannot establish.
