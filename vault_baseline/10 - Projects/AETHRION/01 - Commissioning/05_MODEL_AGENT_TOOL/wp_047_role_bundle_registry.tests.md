...
status: NOT_STARTED
source: "planning/commissioning/05_MODEL_AGENT_TOOL/WP-047_role_bundle_registry.tests.md"
generated: false
...
| Companion | [acceptance criteria](wp_047_role_bundle_registry.acceptance.md) |
...
| C04 | Build the `RoleBundle` schema and its Git registry | WP-047-T01 | *(name the test case)* |
| C05 | Write the `RoleContract` → runtime prompt/tool/context compiler | WP-047-T02 | *(name the test case)* |
| C06 | Create the initial bundles for planner, scout, extractor, methodologist, coder, reviewer, reproducer and curator | WP-047-T03 | *(name the test case)* |
| C07 | Bind the context budget and frozen-package policy | WP-047-T04 | *(name the test case)* |
| C08 | Add bundle signature, admission and evaluation references | WP-047-T05 | *(name the test case)* |
| C09 | Establish deprecation and migration management | WP-047-T06 | *(name the test case)* |
| C10 | Build the **Skill Registry**: discovery, the Agent Skills format contract, and `scripts/validate_skills.py` as an admission gate | WP-047-T07 | *(name the test case)* |
| C11 | Implement **trigger resolution** — classification fields → `skills_required` — with a recorded `skill_selection_reason` | WP-047-T08 | *(name the test case)* |
| C12 | Implement **version and dependency resolution** across `airl.requires_skills`, including conflict refusal | WP-047-T09 | *(name the test case)* |
| C13 | Compute and record `skill_bundle_hash`; bind it into `TaskContract` and the evidence chain | WP-047-T10 | *(name the test case)* |
| C14 | Enforce the **two-family policy**: engineering, scientific-research and shared, selected from `work_domain` — never chosen freely by the agent | WP-047-T11 | *(name the test case)* |
| C15 | Track **upstream provenance**: `airl.derived_from` + `airl.upstream_commit`, and flag derived skills when upstream moves | WP-047-T12 | *(name the test case)* |
...
[acceptance criteria](wp_047_role_bundle_registry.acceptance.md) reaches the decision — issuance is not acceptance.
