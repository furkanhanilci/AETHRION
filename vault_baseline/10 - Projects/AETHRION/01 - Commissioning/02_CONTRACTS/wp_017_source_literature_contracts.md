---
title: "WP-017 — Source Registry and Literature Contract Schemas"
aliases:
  - "WP-017"
...
status: NOT_STARTED
summary: "Source identity, representation, trust, search, screening, set manifest, Zotero binding and status-event schemas are defined canonically, so that a citation means the same thing everywhere in the system."
source: "planning/commissioning/02_CONTRACTS/WP-017_source_literature_contracts.md"
generated: true
...
| [Test procedures](wp_017_source_literature_contracts.tests.md) | How is it tested, in what environment, with what data, and what counts as a complete run? | The implementer and the tester |
| [Acceptance criteria](wp_017_source_literature_contracts.acceptance.md) | What must hold for this to be `ACCEPTED`, and what does it still not establish? | The independent verifier |
...
Neither register binds an upstream mechanism or a runtime component to `WP-017`, so every deliverable below is **`BUILD_NATIVE`**.
...
The outline below is the summary. The executable procedure — environment, data, coverage items, cases, execution log, incident and completion reports — is in [`WP-017_source_literature_contracts.tests.md`](wp_017_source_literature_contracts.tests.md).
...
The programme-level conditions are below. The package-specific, measurable criteria — each with a threshold and the test case that decides it — are in [`WP-017_source_literature_contracts.acceptance.md`](wp_017_source_literature_contracts.acceptance.md), together with what this package still cannot establish.
