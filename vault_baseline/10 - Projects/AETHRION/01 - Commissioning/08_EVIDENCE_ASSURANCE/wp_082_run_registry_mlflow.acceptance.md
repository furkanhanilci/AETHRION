---
title: "WP-082 — Run Registry and MLflow Lineage Integration — Acceptance Criteria"
aliases:
  - "WP-082 acceptance"
cssclasses:
...
status: NOT_STARTED
source: "planning/commissioning/08_EVIDENCE_ASSURANCE/WP-082_run_registry_mlflow.acceptance.md"
generated: false
...
# WP-082 — Run Registry and MLflow Lineage Integration — Acceptance Criteria
...
Each criterion names the test case in [`WP-082_run_registry_mlflow.tests.md`](wp_082_run_registry_mlflow.tests.md) that decides it. A criterion with no test case cannot be verified, and a test case that decides no criterion is not part of acceptance.

<!-- /generated:howto -->

## Package-specific acceptance criteria

- [ ] **Admission refuses a run missing any of the ten manifest elements**, naming
      the element — one transcript per element — and refuses retrospective
      completion.
- [ ] A confirmatory claim from an incomplete run is refused.
- [ ] Every admitted run resolves to its workflow, its execution lease and its
      tracking run in one query.
- [ ] **MLflow holds no run the registry does not.** It is the tracking view; the
      registry is the system-of-record.
- [ ] An artifact whose hash differs from the manifest, and a metric outside the
      schema, are both refused.
- [ ] **Failed, cancelled and negative are three distinguishable outcomes.** A
      failed run carries no result; a cancelled run's partial results are marked
      partial; **a negative result is first-class and citable**, and discarding one
      is refused.
- [ ] Run comparison names exactly which manifest elements differ, and a
      reproduction reports *differs in environment only* checkably.
- [ ] A completed run's metrics cannot be altered, and completion publishes its
      event in the same transaction.

## What this package cannot establish

> **Making negative results storable does not make them published.** `PR-19` is
> that publication bias survives the gate structure — G2 freezes the protocol and
> G8 can still decline to publish on the direction of the result. This package
> removes the technical excuse; the control is the **in-principle acceptance at
> G2** that `00_PROGRAM/01` proposes, and it is a governance decision this package
> cannot make.
