> [!info] Generated view
> This note is generated from `docs/architecture/AIRL_OS_ARCHITECTURE.md` in the repository. Edit the
...
![The trust boundary, with one injected instruction followed to the point where it is denied](figures/airl_os_trust.svg)
...
![The commissioning programme: eleven waves against the one package that has produced anything](figures/airl_os_waves.svg)
...
| **C2** | What "independent verifier" means for a one-person organisation | **Closed by [ADR-001](ADR-001_reviewer_independence.md).** R1 solo, R2 declared-partial, R3 `BLOCKED` — enforced, not argued |
...
| **H5** | No continuous integration | **Staged, not active** — the workflow exists at `deploy/bvc-01-verify.yml` and has never run; see [ADR-002](ADR-002_continuous_verification.md) |
...
![Repository, vault and the outside world, with direction on every edge](figures/airl_os_topology.svg)
...
![What each check proves, and what it cannot see](figures/airl_os_verification.svg)
...
[`AIRL_OS_COMPONENT_REUSE.md`](AIRL_OS_COMPONENT_REUSE.md) — none of which has
been run.
