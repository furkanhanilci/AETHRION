...
| [Test procedures](WP-011_identity_correlation_standard.tests.md) | How is it tested, in what environment, with what data, and what counts as a complete run? | The implementer and the tester |
| [Acceptance criteria](WP-011_identity_correlation_standard.acceptance.md) | What must hold for this to be `ACCEPTED`, and what does it still not establish? | The independent verifier |
...
- Dependencies accepted: [WP-010 — Architecture Decision and Rejected-Alternatives Baseline](../01_GOVERNANCE/WP-010_adr_baseline.md)
...
Neither register binds an upstream mechanism or a runtime component to `WP-011`, so every deliverable below is **`BUILD_NATIVE`**.
...
The outline below is the summary. The executable procedure — environment, data, coverage items, cases, execution log, incident and completion reports — is in [`WP-011_identity_correlation_standard.tests.md`](WP-011_identity_correlation_standard.tests.md).
