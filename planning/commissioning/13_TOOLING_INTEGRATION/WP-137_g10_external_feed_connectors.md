...
| [Test procedures](WP-137_g10_external_feed_connectors.tests.md) | How is it tested, in what environment, with what data, and what counts as a complete run? | The implementer and the tester |
| [Acceptance criteria](WP-137_g10_external_feed_connectors.acceptance.md) | What must hold for this to be `ACCEPTED`, and what does it still not establish? | The independent verifier |
...
| [WP-136 — Inbound Content Quarantine and Channel Allowlist](../13_TOOLING_INTEGRATION/WP-136_inbound_content_quarantine.md) | — |
...
|---|---|---|---|---|
| `ASM-057` — Crossref Retraction Watch and Crossmark — source status signals | `DEPENDENCY` | the running implementation | the contract this is held behind | **2** |
...
|---|---|---|
| `ASM-057` | A retraction signal opens an impact scan. It does not itself retract a claim — that is a decision, recorded as one. | A custom retraction database. Crossref already carries Retraction Watch data and exposes update-to and updated-by relations. |

### Where a plain row would mislead

- **`ASM-057`** — **The one entry in this register with running code.** `scripts/monitor_sources.py` sweeps Crossref and exits non-zero if its planted retracted DOI goes undetected. It also reports its own boundary: 18 of 33 sources carry no DOI and are invisible to it. Material-delta scoring — a citation-count change is not an event, a retraction is — is what keeps G10 from becoming a notification nobody reads (PR-56).
...
**`ASM-057` — Crossref Retraction Watch and Crossmark — source status signals** · `DEPENDENCY` · status `ACCEPTED`
...
The outline below is the summary. The executable procedure — environment, data, coverage items, cases, execution log, incident and completion reports — is in [`WP-137_g10_external_feed_connectors.tests.md`](WP-137_g10_external_feed_connectors.tests.md).
...
The programme-level conditions are below. The package-specific, measurable criteria — each with a threshold and the test case that decides it — are in [`WP-137_g10_external_feed_connectors.acceptance.md`](WP-137_g10_external_feed_connectors.acceptance.md), together with what this package still cannot establish.
