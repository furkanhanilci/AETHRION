...
|---|---|---|---|---|
| `CMP-042` — Grafana | `DEPENDENCY` | Dashboards, alerting UI and datasource plumbing. | Which six operational graphs must exist, and what each is evidence of. | **2** |
...
|---|---|---|
| `CMP-042` | A dashboard displays; it decides nothing. A green dashboard is not an acceptance, and an alert that nobody can act on is not a control. | Grafana as an audit ledger — that is WP-099, and it is WORM. |
...
**`CMP-042` — Grafana** · `DEPENDENCY` · status `PROPOSED`
