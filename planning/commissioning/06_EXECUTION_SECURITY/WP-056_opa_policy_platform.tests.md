...
| Companion | [acceptance criteria](WP-056_opa_policy_platform.acceptance.md) |
...
| Live state | `python3 scripts/progress.py show WP-056` |
...
| Evidence sink | Somewhere `EvidenceManifest` can be issued and verified | Safety / Security / Internal Audit | At completion |
...
| C13 | D3 Data to a Public Provider | [ACC-18](../12_ACCEPTANCE_SCENARIOS/ACC-18_d3_public_route.md) — Critical | *(name the test case)* |
| C14 | Policy Bundle Rollback | [ACC-24](../12_ACCEPTANCE_SCENARIOS/ACC-24_policy_bundle_rollback.md) — High | *(name the test case)* |
| C15 | Approval, Delegation and Exception Expiry | [ACC-26](../12_ACCEPTANCE_SCENARIOS/ACC-26_approval_expiry.md) — Critical | *(name the test case)* |
...
|---|---|---|---|---|
| **TC-01** Module boundaries | E0 | Inspect the policy repository | Eight domains as separate modules with declared precedence | Repository map |
| **TC-02** No cross-product | **E0** | Search for a rule set evaluating all domains at once | None; precedence is explicit | Review record |
| **TC-03** Input document | E0 | Inspect the decision API | Input schema is versioned and validated | Schema |
| **TC-04** Unit tests | E1 | Run the policy unit suite | All pass | Suite report |
| **TC-05** **Negative tests** | **E2** | Confirm every rule has a case that must be **denied** | Zero rules with only positive cases | Coverage report |
| **TC-06** Property tests | **E2** | Run property tests over generated inputs | No input produces an allow the precedence rules forbid | Property run |
| **TC-07** Bundle signature | **E2** | Load an unsigned bundle | Refused at every enforcement point | Refusal per point |
| **TC-08** Bundle promotion | E1 | Promote dev → staging → prod | The **same bundle digest** throughout | Promotion record |
| **TC-09** Rollback | **E1** | Roll back to the previous signed bundle | Restored; the decision log is preserved (`00_PROGRAM/09`) | Rollback transcript |
| **TC-10** **Shadow evaluation** | **E1** | Run a new bundle in shadow | Decisions compared; differences reported; **nothing enforced** | Shadow report |
| **TC-11** Shadow difference | **E2** | Introduce a bundle that flips a decision | The flip appears in the shadow report **before** promotion | Difference record |
| **TC-12** Coverage telemetry | **E1** | Inspect rule firing counts | Rules that have never fired are listed | Coverage report |
| **TC-13** Dead rule | **E2** | Confirm a never-fired rule is investigated, not ignored | Each is a finding with a disposition | Finding records |
| **TC-14** Decision log redaction | **E2** | Evaluate against D3 input | The log records the decision; the D3 content is redacted | Log sample |
| **TC-15** WORM export | **E2** | Attempt to alter an exported decision log | Refused | Refusal transcript |
| **TC-16** Explanation | **E1** | Inspect any denial | Names the module, the rule and the inputs that produced it (WP-016) | Denial record |
...
python3 scripts/progress.py show WP-056 # dependencies and their states
...
```bash
python3 scripts/evidence_manifest.py issue --package WP-056 --gate Platform \
    --subject <each artifact the run produced> \
    --check "<what was verified, in one sentence>"
python3 scripts/evidence_manifest.py verify \
    --manifest delivery/WP-056/evidence.dsse.json --tamper-demo
```
...
```bash
python3 scripts/progress.py tech-complete WP-056
```

`TECH_COMPLETE` is where the producer stops. The verifier named on the
[acceptance criteria](WP-056_opa_policy_platform.acceptance.md) reaches the decision — issuance is not acceptance.
